...
import { 
  ...
  MdFileDownload, MdFilterList, MdTrendingUp, MdLogout
} from 'react-icons/md';
          ...
          <div className="modal-content wide" onMouseDown={(e) => e.stopPropagation()}>
            ...
            <div className="view-profile-body">
               ...
               <div className="gallery-preview">
                 {viewingProvider.images?.map((img, i) => <img key={i} src={img} alt={`${viewingProvider.name} ${i + 1}`} className="gallery-img" />)}
               </div>
            </div>
          </div>
        </div>
